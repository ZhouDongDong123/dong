...
function modifycustomer(obj){
	...
	if(!patternEmail.test(emp_value[5])) {
        yanzheng = false;
        alert("错误的Email格式！");
    }else if(!patternTel.test(emp_value[4])){
    	yanzheng = false;
        alert("错误的手机号！");
    }else if(emp_value[2]=='' || emp_value[2].trim().length == 0 ){
    	yanzheng = false;
        alert("请输入有效地址！");
    }else if(emp_value[3] == '' || emp_value[3].trim().length == 0 ){
    	yanzheng = false;
        alert("请输入有效联系人！");
    }
	if(yanzheng == true){
	...
	<div class="container">
		...
		<div class="row">
			<div class="col-md-4 col-md-offset-8">
				...
				<a class="btn btn-primary" href="jumpAddCustomerPage.action">新增</a>
			...
			<div class="col-md-6">
				...
				<ul class="pagination">
					<!-- 
                      ...
                      -->
					<li><a href="findAllCustomers.action?pn=1">首页</a></li>
					<!-- 如果还有前页就访问当前页码-1的页面， -->
					<c:if test="${pageInfo.hasPreviousPage}">
						<li><a
							href="findAllCustomers.action?pn=${pageInfo.pageNum-1}"
					...
					<li>
						<!--遍历所有导航页码，如果遍历的页码页当前页码相等就高亮显示，如果相等就普通显示  --> <c:forEach
							...
							<c:if test="${page_Nums!=pageInfo.pageNum }">
								<li><a href="findAllCustomers.action?pn=${page_Nums}">${page_Nums}</a></li>
							</c:if>
						</c:forEach>
					</li>
					<!-- 如果还有后页就访问当前页码+1的页面， -->
					...
					<li><a href="findAllCustomers.action?pn=${pageInfo.pages}">末页</a></li>
				</ul>
				</nav>
			</div>
		</div>
